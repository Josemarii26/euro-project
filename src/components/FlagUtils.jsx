export const getTeamFlag = (teamName) => {
  switch (teamName) {
    case "turkey":
      return "https://upload.wikimedia.org/wikipedia/commons/b/b4/Flag_of_Turkey.svg";
    case "italy":
      return "https://upload.wikimedia.org/wikipedia/en/0/03/Flag_of_Italy.svg";
    case "wales":
      return "https://upload.wikimedia.org/wikipedia/commons/5/59/Flag_of_Wales_2.svg";
    case "switzerland":
      return "https://upload.wikimedia.org/wikipedia/commons/0/08/Flag_of_Switzerland_%28Pantone%29.svg";
    case "denmark":
      return "https://upload.wikimedia.org/wikipedia/commons/9/9c/Flag_of_Denmark.svg";
    case "finland":
      return "https://upload.wikimedia.org/wikipedia/commons/b/bc/Flag_of_Finland.svg";
    case "belgium":
      return "https://upload.wikimedia.org/wikipedia/commons/6/65/Flag_of_Belgium.svg";
    case "netherlands":
      return "https://upload.wikimedia.org/wikipedia/commons/2/20/Flag_of_the_Netherlands.svg";
    case "ukraine":
      return "https://upload.wikimedia.org/wikipedia/commons/4/49/Flag_of_Ukraine.svg";
    case "austria":
      return "https://upload.wikimedia.org/wikipedia/commons/4/41/Flag_of_Austria.svg";
    case "north macedonia":
      return "https://upload.wikimedia.org/wikipedia/commons/f/f8/Flag_of_North_Macedonia.svg";
    case "england":
      return "https://upload.wikimedia.org/wikipedia/en/b/be/Flag_of_England.svg";
    case "croatia":
      return "https://upload.wikimedia.org/wikipedia/commons/1/1b/Flag_of_Croatia.svg";
    case "scotland":
      return "https://upload.wikimedia.org/wikipedia/commons/1/10/Flag_of_Scotland.svg";
    case "czech republic":
      return "https://upload.wikimedia.org/wikipedia/commons/c/cb/Flag_of_the_Czech_Republic.svg";
    case "spain":
      return "https://upload.wikimedia.org/wikipedia/en/9/9a/Flag_of_Spain.svg";
    case "sweden":
      return "https://upload.wikimedia.org/wikipedia/en/4/4c/Flag_of_Sweden.svg";
    case "poland":
      return "https://upload.wikimedia.org/wikipedia/en/1/12/Flag_of_Poland.svg";
    case "slovakia":
      return "https://upload.wikimedia.org/wikipedia/commons/e/e6/Flag_of_Slovakia.svg";
    case "hungary":
      return "https://upload.wikimedia.org/wikipedia/commons/c/c1/Flag_of_Hungary.svg";
    case "portugal":
      return "https://upload.wikimedia.org/wikipedia/commons/5/5c/Flag_of_Portugal.svg";
    case "france":
      return "https://upload.wikimedia.org/wikipedia/en/c/c3/Flag_of_France.svg";
    case "germany":
      return "https://upload.wikimedia.org/wikipedia/en/b/ba/Flag_of_Germany.svg";
    case "albania":
      return "https://upload.wikimedia.org/wikipedia/commons/3/36/Flag_of_Albania.svg";
    case "serbia":
      return "https://upload.wikimedia.org/wikipedia/commons/f/ff/Flag_of_Serbia.svg";
    case "slovenia":
      return "https://upload.wikimedia.org/wikipedia/commons/f/f0/Flag_of_Slovenia.svg";
    case "romania":
      return "https://upload.wikimedia.org/wikipedia/commons/7/73/Flag_of_Romania.svg";
    case "georgia":
      return "https://upload.wikimedia.org/wikipedia/commons/0/0f/Flag_of_Georgia.svg";
    case "bosnia and herzegovina":
      return "https://upload.wikimedia.org/wikipedia/commons/b/bf/Flag_of_Bosnia_and_Herzegovina.svg";
    case "norway":
      return "https://upload.wikimedia.org/wikipedia/commons/d/d9/Flag_of_Norway.svg";
    case "greece":
      return "https://upload.wikimedia.org/wikipedia/commons/5/5c/Flag_of_Greece.svg";
    case "luxembourg":
      return "https://upload.wikimedia.org/wikipedia/commons/d/da/Flag_of_Luxembourg.svg";
    case "kosovo":
      return "https://upload.wikimedia.org/wikipedia/commons/1/1f/Flag_of_Kosovo.svg";
    case "moldova":
      return "https://upload.wikimedia.org/wikipedia/commons/2/27/Flag_of_Moldova.svg";
    case "montenegro":
      return "https://upload.wikimedia.org/wikipedia/commons/6/64/Flag_of_Montenegro.svg";
    case "estonia":
      return "https://upload.wikimedia.org/wikipedia/commons/8/8f/Flag_of_Estonia.svg";
    case "latvia":
      return "https://upload.wikimedia.org/wikipedia/commons/8/84/Flag_of_Latvia.svg";
    case "lithuania":
      return "https://upload.wikimedia.org/wikipedia/commons/1/11/Flag_of_Lithuania.svg";
    case "malta":
      return "https://upload.wikimedia.org/wikipedia/commons/7/73/Flag_of_Malta.svg";
    case "bulgaria":
      return "https://upload.wikimedia.org/wikipedia/commons/9/9a/Flag_of_Bulgaria.svg";
    default:
      return "";
  }
};
